...
- **Gear Sale & Rehoming** (`gear.html`): listing cards live in `.gear-grid`, one `.gear-card` per item (image, condition badge, price, description, WhatsApp CTA). To mark an item **sold**, follow the template in the HTML comment above `.gear-grid` — add `class="sold"` to the card, a `.gear-sold-badge`, and a `.sold-note`; this grays out the photo, dims the price, and hides the Enquire button automatically (all CSS-driven, no JS). There's no admin panel — marking something sold is a normal edit+push, same as everything else on this site.
