...
- **Guests Gallery** (`guests.html`): currently an `.empty-state` block. Once real guest photos are approved, replace it with a `.gallery-grid` of `<figure>` entries (copy the pattern used in `gallery.html`).
- **Open Graph / social share previews**: every public page has `og:*` and `twitter:*` meta tags right after its `<meta name="description">` — title, description and a representative image, so sharing a link on WhatsApp/Instagram/etc. shows a proper preview card. **If the site moves to a custom domain**, these need updating: every `og:url` and `og:image` currently points at `https://pradeepmurthy1992.github.io/Mugil_wildpixels/` — find-and-replace that base URL across all `.html` files with the new domain.
